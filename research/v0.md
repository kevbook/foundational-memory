...
Large Language Models (LLMs) and autonomous agents are fundamentally constrained by limited context windows and their reliance on static embedding representations whose semantic fidelity degrades over time.
...
flowchart TB
    subgraph Components["SYSTEM COMPONENTS"]
        ...
        subgraph CRE["4. CROSS-LAYER RETRIEVAL ENGINE"]
            CRE_D["Vector → Graph → Surface pipeline<br/>Ranks and synthesizes results"]
        end
...
| **Cross-Layer Retrieval**      | Vector → Graph → Surface pipeline; rank and synthesize results           |
...
flowchart LR
    subgraph Discrete["DISCRETE (Graph)"]
        N["Node nᵢ<br/>(memory unit)"]
        E["Edge eᵢⱼ<br/>(relation)"]
        P["Path P<br/>(n₁→n₂→...→nₖ)"]
    end

    subgraph Bridge["BRIDGE"]
        PHI["φ(nᵢ)<br/>(encoder)"]
        PSI["ψ(eᵢⱼ)<br/>(encoder)"]
        GAMMA["Γ(P)<br/>(compose)"]
    end

    subgraph Continuous["CONTINUOUS"]
        V["vᵢ ∈ ℝᵈ<br/>(embedding)"]
        R["rᵢⱼ ∈ ℝᵈ<br/>(relation vector)"]
        T["trajectory τ<br/>(manifold curve)"]
    end
...
**Inverse Mapping (Continuous → Discrete):**
...
| -------------- | --------------------------------------------------- | -------------------------------------- |
| `CAUSAL`       | \( n_i \) caused or led to \( n_j \)                | "User clicked button" → "Modal opened" |
...
```
INSERT(content, metadata, relations) → node_id
...
QUERY_SYMBOLIC(pattern) → {nodes}
  Graph pattern matching (e.g., Cypher-like queries)

QUERY_GEOMETRIC(vector, k, threshold?) → {(node, score)}
...
Rather than storing three separate bullet points, the system forms a _surface_ in "coding preferences space" that encodes: "User prefers explicit structure → strong bias away from arrow syntax → favors function declarations." This surface enables correct behavior inference even for cases not explicitly stored.
...
```mermaid
flowchart LR
    subgraph BS["BÉZIER SURFACE MODEL"]
...
flowchart TB
    subgraph EEM["EMBEDDING EVOLUTION MODEL"]
        ...
        subgraph CON["4. CONSOLIDATION (Sleep-like)"]
            ...
            CON_1["• Merge similar nodes"]
            CON_2["• Strengthen frequently co-activated paths"]
            CON_3["• Prune low-salience nodes"]
            CON_4["• Re-embed based on accumulated context"]
        end
    end
...
flowchart TB
    ...
    subgraph FL["FUSION LAYER"]
        SCORE["score = α·graph_score + (1-α)·vector_score<br/>+ γ·path_bonus + δ·recency_bonus"]
    end
...
flowchart TB
    ...
    subgraph Step2["Step 2: Graph Traversal"]
        N1["🗣️ User mentioned<br/>budget is $50k"]
        N2["🤖 Agent proposed<br/>Plan B option"]
        N3["✅ User approved<br/>Plan B"]
...
- _"Why did you answer that?"_ → trace the retrieval path
- _"Which facts influenced this?"_ → list contributing nodes
- _"Where did that memory come from?"_ → follow provenance edges
...
> **Embeddings → Vector Store (semantic) → Graph (structured) → Reasoning**
